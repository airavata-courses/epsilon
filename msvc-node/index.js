    ...
    res.errorBhejdo = function (status, body = {}, acfBhejnaHai, custom_error) {
        ...
        if (!custom_error) {
            if (body && body.stack) {
                ...
                log.logError('Error toh aaya hai, par jabh tak bhejoge nai toh dikhau kaise? Please use the last parameter(Error Object) to send an error object or atleast include error in body.msg(string) or body.stack(string).');
            }
        } else {
            if (custom_error.message && custom_error.stack)
                log.logError(custom_error);
            else
                if (typeof custom_error == "string") log.logCustomError(custom_error)
                else if (typeof custom_error == "object") log.logCustomError(JSON.stringify(custom_error))
                else log.logError('Custom error toh daal diye ho par atleast Error Object bana lo ya toh string ya toh Object, yeh kya bavaseer bhej diye ho');
        }
...
app.use('*', (req, res, next) => {
    ...
    if (req.headers.source === 'webapp' && !result &&
        (process.env.SERVER_SETUP === 'production' || process.env.SERVER_SETUP === 'staging')) {
